#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────────
# Balanciz — Fresh Install Script for Ubuntu 24.04 LTS
# ─────────────────────────────────────────────────────────────────────────────
#
# Usage:
...
#   2. Creates a "balanciz" system user
#   3. Creates the PostgreSQL database and role
#   4. Builds Balanciz from source (Go binaries + CSS + React static assets)
...
INSTALL_DIR="/opt/balanciz"
LEGACY_INSTALL_DIR="/opt/gobooks"
DATA_DIR="/opt/balanciz/data"
BACKUP_DIR="/var/backups/balanciz"
GO_VERSION="1.26.1"
NODE_INSTALL_MAJOR=22
SERVICE_USER="balanciz"
DB_NAME="balanciz"
DB_USER="balanciz"
APP_PORT=6768
...
log()  { echo -e "${GREEN}[Balanciz]${NC} $*"; }
warn() { echo -e "${YELLOW}[Balanciz]${NC} $*"; }
err()  { echo -e "${RED}[Balanciz]${NC} $*" >&2; }
...
if [[ -f "$INSTALL_DIR/.env" ]] || [[ -f "$INSTALL_DIR/bin/balanciz" ]]; then
...
if [[ -f "$LEGACY_INSTALL_DIR/.env" ]] || [[ -f "$LEGACY_INSTALL_DIR/bin/gobooks" ]]; then
    err "Legacy Gobooks installation detected at ${LEGACY_INSTALL_DIR}."
    err "Run upgrade.sh to upgrade the existing deployment in place."
    exit 1
fi
...
if [[ "$USE_DEFAULTS" == true ]]; then
    ...
    echo ""
    echo -e "${CYAN}── Balanciz Installation ──${NC}"
    echo ""
...
CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -o bin/balanciz         ./cmd/balanciz
CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -o bin/balanciz-migrate  ./cmd/balanciz-migrate
...
cat > "$ENV_FILE" <<ENVEOF
# Balanciz production configuration — generated by install.sh
APP_ENV=prod
APP_ADDR=:${APP_PORT}
LOG_LEVEL=INFO
...
cat > /etc/systemd/system/balanciz.service <<SVCEOF
[Unit]
...
ExecStartPre=${INSTALL_DIR}/bin/balanciz-migrate
ExecStart=${INSTALL_DIR}/bin/balanciz
Restart=on-failure
RestartSec=5
...
systemctl enable balanciz
systemctl start balanciz

log "Balanciz service started."

# ── 11. Configure Nginx reverse proxy ────────────────────────────────────────

log "Configuring Nginx reverse proxy..."
cat > /etc/nginx/sites-available/balanciz <<'NGXEOF'
server {
...
ln -sf /etc/nginx/sites-available/balanciz /etc/nginx/sites-enabled/balanciz
rm -f /etc/nginx/sites-enabled/default

nginx -t && systemctl reload nginx
log "Nginx configured (port 80 → Balanciz)."

# ── 12. Setup daily backup cron ──────────────────────────────────────────────

log "Setting up daily backup cron..."
cat > /etc/cron.d/balanciz-backup <<CRONEOF
# Balanciz daily database backup at 02:00 AM, keep 30 days
0 2 * * * postgres pg_dump ${DB_NAME} | gzip > ${BACKUP_DIR}/balanciz_\$(date +\%Y\%m\%d).sql.gz 2>/dev/null
0 3 * * * root find ${BACKUP_DIR} -name "balanciz_*.sql.gz" -mtime +30 -delete 2>/dev/null
CRONEOF

chmod 644 /etc/cron.d/balanciz-backup
...
echo ""
echo -e "${GREEN}════════════════════════════════════════════════════════════════${NC}"
echo -e "${GREEN}  Balanciz installed successfully!${NC}"
echo -e "${GREEN}════════════════════════════════════════════════════════════════${NC}"
echo ""
echo -e "  Open:     ${CYAN}http://$(hostname -I | awk '{print $1}')${NC}"
echo -e "  Status:   ${CYAN}sudo systemctl status balanciz${NC}"
echo -e "  Logs:     ${CYAN}sudo journalctl -u balanciz -f${NC}"
