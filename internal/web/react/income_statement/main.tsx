...
type FlatRow =
  | { kind: "section"; key: string; section: ISSection }
  | { kind: "line"; key: string; section: ISSection; line: ISLine }
  | { kind: "total"; key: string; section: ISSection }
  | { kind: "summary"; key: string; label: string; amount: string };
...
function buildRows(data: ISResponse, query: string): FlatRow[] {
  const rows: FlatRow[] = [];
  const revenue = data.sections.find((section) => section.root === "revenue");
  const cogs = data.sections.find((section) => section.root === "cost_of_sales");
  const expenses = data.sections.find((section) => section.root === "expense");
  for (const section of [revenue, cogs].filter(Boolean) as ISSection[]) {
    appendSection(rows, section, query);
  }
  rows.push({ kind: "summary", key: "gross_profit", label: "Gross Profit", amount: data.totals.gross_profit });
  if (expenses) appendSection(rows, expenses, query);
  rows.push({ kind: "summary", key: "net_income", label: "Net Income", amount: data.totals.net_income });
  return rows;
}

function appendSection(rows: FlatRow[], section: ISSection, query: string) {
  if (!sectionMatches(section, query)) return;
...
async function fetchJSON(url: string): Promise<ISResponse> {
  ...
  if (!response.ok) throw new Error(`Income Statement request failed with ${response.status}`);
  return (await response.json()) as ISResponse;
}

function IncomeStatementExplorer({ apiURL }: { apiURL: string }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "Income Statement could not be loaded.");
    } finally {
      setLoading(false);
    }
  }, [apiURL]);
  ...
  if (!data && loading) {
    return <Shell><div className="rounded-lg border border-border bg-surface p-8 text-center text-small text-text-muted2">Loading Income Statement...</div></Shell>;
  }
  if (!data) {
    return <Shell><ErrorBox message={error || "Income Statement could not be loaded."} /></Shell>;
  }
  ...
  return (
    <Shell>
      ...
      <section className="grid grid-cols-1 gap-3 lg:grid-cols-5">
        <Metric label="Revenue" value={fmt(data.totals.revenue)} />
        <Metric label="Cost of Sales" value={fmt(data.totals.cost_of_sales)} />
        <Metric label="Gross Profit" value={fmt(data.totals.gross_profit)} />
        <Metric label="Expenses" value={fmt(data.totals.expenses)} />
        <Metric label="Net Income" value={fmt(data.totals.net_income)} tone={money(data.totals.net_income) >= 0 ? "good" : "bad"} />
      </section>
      ...
      <section className="rounded-lg border border-border bg-surface shadow-sm">
        <div className="flex flex-wrap items-end gap-3 border-b border-border px-4 py-3">
          ...
          <span className="text-small text-text-muted2">{data.source ? `Freshness: ${data.source}` : ""}</span>
        ...
        <div className="overflow-x-auto">
          <div className="min-w-[860px]">
            ...
            <div
              ...
              data-react-income-statement-ready="true"
            >
...
function Shell({ children }: { children: React.ReactNode }) {
  return <div className="mt-4 space-y-3" data-react-income-statement-shell="true">{children}</div>;
}
...
function mount(root: HTMLElement) {
  const apiURL = root.dataset.apiUrl || "/api/reports/income-statement";
  createRoot(root).render(<IncomeStatementExplorer apiURL={apiURL} />);
}

document.querySelectorAll<HTMLElement>('[data-gb-react="income-statement"]').forEach((root) => {
  mount(root);
});
