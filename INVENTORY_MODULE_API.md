...
| **IN.8** | Rule #4 post-time invariant extension to Receipt (H.3) and Shipment (I.3) post paths. Adds `Rule4DocReceipt` (owner iff `receipt_required=true`, mirror of Bill surrender) + `Rule4DocShipment` (owner iff `shipment_required=true`, mirror of Invoice surrender). Post-time assertion now runs at the tail of `PostReceipt` + `PostShipment` to catch silent-swallow regressions symmetric to Bill / Invoice / Expense / CN / VCN / ARR / VRS coverage. 7 contract tests (dispatch + silent-swallow catch + non-owner pass). Closes the "Does NOT run on Receipt/Shipment posts today" gap pre-IN.8 `rule4_invariant.go` called out. | IN.7 shipped | shipped (local) |
